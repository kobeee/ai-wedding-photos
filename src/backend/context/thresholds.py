...
FINAL_FACE_AREA_FLOOR_COUPLE = 0.025
FINAL_FACE_AREA_FLOOR_SOLO = 0.035
FINAL_BODY_VISIBILITY_FLOOR = 0.65
FINAL_BODY_VISIBILITY_FLOOR_SOLO = 0.55
...
def passes_final_delivery_gate(
    verifiability: VerifiabilityAssessment,
    *,
    gender: str,
) -> bool:
    is_solo = gender in ("female", "male")
    face_ratios = _relevant_face_ratios(verifiability, gender)
    face_floor = FINAL_FACE_AREA_FLOOR_SOLO if is_solo else FINAL_FACE_AREA_FLOOR_COUPLE
    body_floor = FINAL_BODY_VISIBILITY_FLOOR_SOLO if is_solo else FINAL_BODY_VISIBILITY_FLOOR
    return (
        verifiability.is_identity_verifiable
        and verifiability.is_proportion_verifiable
        and all(ratio >= face_floor for ratio in face_ratios)
...
def summarize_final_delivery_failure(
    verifiability: VerifiabilityAssessment,
    *,
    gender: str,
) -> str:
    ...
    if not verifiability.is_identity_verifiable:
        return "Identity is not verifiable"
    if not verifiability.is_proportion_verifiable:
        return "Body proportion is not verifiable"
    is_solo = gender in ("female", "male")
    face_floor = FINAL_FACE_AREA_FLOOR_SOLO if is_solo else FINAL_FACE_AREA_FLOOR_COUPLE
    if min_face_ratio < face_floor:
        return f"Face area ratio {min_face_ratio:.3f} below floor {face_floor:.3f}"
    body_floor = FINAL_BODY_VISIBILITY_FLOOR_SOLO if is_solo else FINAL_BODY_VISIBILITY_FLOOR
    if verifiability.body_visibility_score < body_floor:
        return (
            f"Body visibility {verifiability.body_visibility_score:.2f} "
            f"below floor {body_floor:.2f}"
        )
    if verifiability.notes:
        return verifiability.notes[0]
    return "Final delivery gate failed"
